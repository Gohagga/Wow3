...
export interface MoonfireConfig extends Wc3AbilityData {
    sfxModelPath: string,
}

type MoonfireUnitData = {
    Damage: number,
    Range: number,
    Cost: number,
    Cooldown: number,
...
export class Moonfire extends AbilityBase implements IUnitConfigurable<MoonfireUnitData> {

    public unitConfig = new UnitConfigurable<MoonfireUnitData>(() => ({
        Damage: 20,
        Range: 1000,
        Cost: 13,
        Cooldown: 4.5,
    ...
    UpdateUnitConfig(unit: Unit, cb: (this: void, config: MoonfireUnitData) => void): void {
